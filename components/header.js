  ...
  return (
      ...
      <nav ref={navRef} className="bg-white drop-shadow-lg relative  z-20  w-full top-0 items-center justify-center">
        <div className="items-center  max-w-screen-xl mx-auto md:px-8 lg:flex">
          ...
          <div className={`items-center content-center justify-center mx-auto ${state ? 'block' : 'hidden'} lg:flex lg:mx-0`}>
            <ul className="justify-center items-center space-y-8 lg:flex gap-24 lg:space-y-0">
              {
                navigation.map((item, idx) => {
                  return (
                    <li key={idx} className="text-gray-600 hover:text-primary-300 sm:text-sm md:text-sm lg:text-sm xl:text-md justify-center items-center flex">
                      <a href={item.path}>
                        {item.title}
                      </a>
                    </li>
